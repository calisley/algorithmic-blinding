...
df <- read.csv("./data/stronger_discrim/final_data_for_analysis.csv")
...
# Keep only columns that don't match the pattern
df_clean <- df[, !grepl(pattern, cols)]
df_clean_f <- df_clean %>% select(-c(`ID.1`, Resume_str, Resume_html))
# Write the cleaned data to a new CSV file
write.csv(df_clean_f, "./data/stronger_discrim/final_data_for_analysis_cleaned.csv", row.names = FALSE)
