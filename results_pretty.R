...
df_s <- read.csv("./data/stronger_discrim/final_data_for_analysis_cleaned.csv")%>% mutate(
  iters_to_success = case_when(
    ...
    !is.na(judge_cosine_1) ~ 1,
    !is.na(judge_cosine_0) ~ 0
  )) %>% filter(race != "", gender != "", !is.na(race)) %>% mutate(change_score = rating_post - rating_pre) %>% 
  filter(race != "Native American", race != "Pacific Islander", race != "Middle Eastern") %>% #only one of each, cant get good stats
  filter(gender != "Nonbinary") #only two, same as above
...
extract_last_non_null_key <- function(row, key) {
  parsed_values <- lapply(row, function(x) {
    ...
    cleaned_x <- gsub('\"\"', '\"', cleaned_x)     # Attempt to parse the JSON; return NULL if it fails
...
# Create the plot
race_bars <- ggplot(summary_df_race_s %>% na.omit(), aes(x = race, y = mean_rating, fill = time)) +
...
###### Appendix #####
df_s$cosine_dist <- apply(df_s[, c("judge_cosine_1", "judge_cosine_2", "judge_cosine_3", "judge_cosine_4")], 1, function(row) {
...
mod<- lm(change_score ~ cosine_dist, data = df_s)

stargazer(mod, type="html", out ='./appendix_table_1.html')
system(paste("wkhtmltoimage", './appendix_table_1.html', "./results/reg_1.png"))
